...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的飞机订单 </p>
    </div>
    <!-- 标题 -->
        ...
        <div class="right-container" style="padding-top: 0;">
            <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
                <ul class="layui-tab-title">
                     <li v-for="item in feijiOrderTypesList" :value="item.codeIndex"  @click="pageList(item.codeIndex)">{{item.indexName}}</li>
                ...
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td style="width: 60px;">{{item.feijiName}}</td>
                            <td style="width: 80px;">{{item.feijiValue}}</td>
                            <td style="width: 80px;">{{item.feijiLeixingValue}}</td>
                            <td style="width: 60px;">{{item.feijiStartTime}}</td>
                            <td style="width: 60px;">{{item.feijiStartAddress}}</td>
                            <td style="width: 60px;">{{item.feijiEndAddress}}</td>
                            <td style="width: 100px;color: red">{{item.feijiNewMoney}}</td>
                            <td style="width: 250px;">
                                <img :src="item.feijiPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            </td>
                            <td style="width: 150px;">{{item.feijiOrderTime}}</td>
                            <td style="width: 80px;">{{item.feijiOrderValue}}</td>
                            <td style="width: 150px;">{{item.insertTime}}</td>
                            <td style="width: 100px;">
                                <button v-if="item.feijiOrderTypes==1" @click="pay(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius btn-theme">
                                    <i class="layui-icon">&#xe65e;</i> 支付
                                </button>
                                <button v-if="item.feijiOrderTypes==2" @click="refund(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 feijiOrderContent 字段去除无用字符 如果字段长度大于60  就截取到60
            feijiOrderContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            //列表查询
            ,pageList(feijiOrderTypes) {
                this.feijiOrderTypes = feijiOrderTypes;
                // 获取列表数据
                layui.http.request('feijiOrder/list', 'get', {
                    page: 1,
                    limit: this.limit,
                    feijiOrderTypes: vue.feijiOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('feijiOrder/list', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    feijiOrderTypes: this.feijiOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                // 获取商品详情信息
                layui.http.requestJson(`feijiOrder/pay`, 'PPST',{
                    id:id
                }, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
            ...
            ,refund(id) {
                // 获取商品详情信息
                layui.http.request(`feijiOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        // 获取列表数据
        http.request('feijiOrder/list', 'get',{
            page: 1,
            limit: limit,
            feijiOrderTypes: vue.feijiOrderTypes
        }, function(res) {
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('feijiOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            feijiOrderType: vue.feijiOrderType
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
       ...
       function feijiOrderTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=feiji_order_types", 'get', {}, function (res) {
               if(res.code == 0){
