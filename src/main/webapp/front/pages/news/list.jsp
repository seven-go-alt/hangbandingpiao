                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="newsName" id="newsName" placeholder="新闻名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                            ...
                            <ul class="after">
                                <li v-for="(item,index) in dataList" v-bind:key="index">
                                    <a target="_self" :href="'../news/detail.jsp?id='+item.id">
                                        <img width="180" class="fl" :src="item.newsPhoto" />
                                        <h5>{{item.newsName}}</h5>
                                        <p class="time-sour">
                                            <span>发布时间：{{item.insertTime}} </span>
                                        </p>
                                        <p class="reamk">{{item.newsContent|newsContent}}...</p>
                                    </a>
                                </li>
                            </ul>
                            <div class="pages">
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 newsContent 字段去除无用字符 如果字段长度大于60  就截取到60
            newsContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#newsName').val() != null && jquery('#newsName').val() != "" && jquery('#newsName').val() != "null" ) {
                param['newsName'] = jquery('#newsName').val();
            }
         
            var newsTypes = $("#newsTypes").find("option:selected").val();
            if (newsTypes != null && newsTypes != "" && newsTypes != "null") {
                param['newsTypes'] = newsTypes;
            }
                                            // 获取列表数据
            http.request('news/list', 'get', param, function(res) {
